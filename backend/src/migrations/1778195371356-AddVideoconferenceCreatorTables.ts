...
export class AddVideoconferenceCreatorTables1778195371356 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`manual_videoconferences\` (\`id\` varchar(36) NOT NULL, \`created_by_user_id\` varchar(36) NOT NULL, \`zoom_group_id\` varchar(36) NOT NULL, \`assigned_zoom_user_id\` varchar(36) NULL, \`backup_zoom_user_id\` varchar(36) NULL, \`type\` enum ('UNIQUE', 'WEEKLY') NOT NULL, \`topic\` varchar(300) NOT NULL, \`agenda\` text NULL, \`start_time\` datetime NOT NULL, \`end_time\` datetime NOT NULL, \`duration_minutes\` int UNSIGNED NOT NULL, \`recurrence_json\` json NULL, \`status\` enum ('CREATED', 'DRAFT_NO_HOST', 'APPROVED_WITH_BACKUP', 'ERROR', 'CANCELLED') NOT NULL DEFAULT 'CREATED', \`zoom_meeting_id\` varchar(50) NULL, \`join_url\` varchar(512) NULL, \`start_url\` varchar(1024) NULL, \`zoom_payload_json\` json NULL, \`zoom_response_json\` json NULL, \`error_message\` text NULL, \`created_at\` datetime NOT NULL, \`updated_at\` datetime NOT NULL, INDEX \`IDX_e5be634be8c8bb278a59b314c5\` (\`start_time\`), INDEX \`IDX_8304851251f059c6d8037be0f8\` (\`status\`), INDEX \`IDX_2ad921f4681737bee33736b6cd\` (\`zoom_group_id\`), INDEX \`IDX_b8b8ed173b702af5b4c6125d8a\` (\`created_by_user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`manual_videoconference_user_zoom_groups\` (\`id\` varchar(36) NOT NULL, \`user_id\` varchar(36) NOT NULL, \`zoom_group_id\` varchar(36) NOT NULL, \`is_active\` tinyint NOT NULL DEFAULT 1, \`created_at\` datetime NOT NULL, \`updated_at\` datetime NOT NULL, INDEX \`IDX_d218cf6de8f42e582784a29d28\` (\`user_id\`), UNIQUE INDEX \`IDX_2cca2fd1c7ffb68953e61b482b\` (\`user_id\`, \`zoom_group_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`meeting_instances\` ADD \`manual_videoconference_id\` varchar(36) NULL`);
        await queryRunner.query(`ALTER TABLE \`zoom_config\` ADD \`ti_alert_emails\` text NULL`);
        await queryRunner.query(`ALTER TABLE \`zoom_groups\` ADD \`backup_zoom_user_id\` varchar(36) NULL`);
